...
class Charcoal extends FlamableGrain {
	constructor (chanceToBurnOut = 0.001, chanceToTurnIntoDirt = 0.001, name = "Charcoal"){
		super(0, 0, 5, 0, name);
...
class WetDirt extends LiquidAffectable {
	constructor(wetGrain = null, dryGrain = null, chanceToGrowGrass = 0.0001, chanceToGrowTree = 0.00003, name = "Wet Dirt") {
						...
						if (this.diesFrom && this.diesFrom.length > 0) {
							for (var i = 0; i < this.diesFrom.length; i++) {
								if (grainObj.type instanceof this.diesFrom[i]) {
									result[1][1] = this.meat[Math.floor(Math.random() * this.meat.length)].getGrainInt(); // Turns into meat
								}
							}
						}
...
class Spider extends FlamableGrain {
    constructor(flammability = 50, chanceToDuplicate = 0.05, chanceToMove = 0.04, chanceToPlaceSilk = 0.0015, name = "Spider") {
